...
exports.createVisitorVerient = async (req, res) => {
  try {
    ...
    if (existing) {
      return res
        .status(400)
        .json({ message: "VisitorVerient code already exists." });
    }
    ...
    res
      .status(201)
      .json({ message: "VisitorVerient created successfully.", data });
  } catch (error) {
...
exports.getAllVisitorVariant = async (req, res) => {
  try {
...
exports.getVisitorVerientByName = async (req, res) => {
  try {
    ...
    const Verient = await VisitorVerient.findOne({
      visitorVerientName: visitorVerientName,
    });
    if (!Verient) {
...
exports.updateVisitorVerient = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "VisitorVerient updated successfully", data: updated });
  } catch (error) {
