...
exports.createCountry = async (req, res) => {
    try {
        const { CountryCode, CountryName,StateID, isActive } = req.body;
...
if (!existingState) {
    return res.status(400).json({ message: 'Invalid StateID' });
        ...
        const country = new Country({ CountryCode, CountryName,StateID , isActive });
        await country.save();
...
exports.updateCountry = async (req, res) => {
    try {
        const {_id,CountryCode,CountryName,StateID,isActive} = {...req.body};
        const country = await Country.findByIdAndUpdate(
            _id,
            { $set:{CountryCode,CountryName,StateID,isActive}},
